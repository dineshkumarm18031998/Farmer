-- ═══════════════════════════════════════════════════════════════
-- 🟡 மஞ்சள் தம்பி — PostgreSQL Schema v3 (FIXED)
-- IMPORTANT: Run column additions BEFORE data migrations
-- ═══════════════════════════════════════════════════════════════
...
-- NOTE: NO NOT NULL on period/paid_date — must support old data migration
...
-- ── STEP 2: ADD MISSING COLUMNS (all ADD before any UPDATE) ───
-- employees
...
-- payments — ADD note FIRST (critical — was missing from old schema)
...
-- ── STEP 3: DATA MIGRATIONS (only after columns exist) ────────
-- Fix paid_date from old 'date' column
UPDATE payments SET paid_date    = date      WHERE (paid_date IS NULL OR paid_date = '') AND date IS NOT NULL;
UPDATE payments SET period_from  = paid_date WHERE (period_from IS NULL OR period_from = '');
UPDATE payments SET period_to    = paid_date WHERE (period_to IS NULL OR period_to = '');
...
-- payments table          = history log only (not used for balance calc)
-- STATUS                  = present | halfday | overtime | leave (no absent)
-- ═══════════════════════════════════════════════════════════════
