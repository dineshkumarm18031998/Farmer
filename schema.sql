-- ═══════════════════════════════════════════════════════════════
-- 🟡 மஞ்சள் தம்பி — PostgreSQL Schema v3 FINAL
-- ═══════════════════════════════════════════════════════════════
...
-- payments: add ALL columns including old 'date' and 'period' for compatibility
ALTER TABLE payments ADD COLUMN IF NOT EXISTS date         TEXT DEFAULT '';
ALTER TABLE payments ADD COLUMN IF NOT EXISTS period       TEXT DEFAULT '';
...
-- ── STEP 3: CRITICAL — DROP ALL NOT NULL ON PAYMENTS ──────────
-- The old schema had: date TEXT NOT NULL, period TEXT NOT NULL
-- These MUST be nullable for the new INSERT to work
ALTER TABLE payments ALTER COLUMN date         DROP NOT NULL;
ALTER TABLE payments ALTER COLUMN period       DROP NOT NULL;
...
-- Sync paid_date from date column
UPDATE payments SET paid_date   = date      WHERE (paid_date  IS NULL OR paid_date  = '') AND date IS NOT NULL AND date != '';
UPDATE payments SET period_from = paid_date WHERE (period_from IS NULL OR period_from = '') AND paid_date IS NOT NULL AND paid_date != '';
UPDATE payments SET period_to   = paid_date WHERE (period_to   IS NULL OR period_to   = '') AND paid_date IS NOT NULL AND paid_date != '';

-- Fill attendance wages
...
-- Compute daily_salary for existing records
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name='advances') THEN
        ...
        SELECT id,emp_id,date,date,amount,date,date,'முன்பணம்',COALESCE(note,''),'advance'
