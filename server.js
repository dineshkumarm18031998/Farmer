...
async function initDB() {
  ...
  // Fix migrations — ADD note column before advances migration
  const migrations = [
    // attendance
    ...
    // CRITICAL: Remove NOT NULL constraints — old schema had date NOT NULL
    ...
    // Also drop NOT NULL on old 'date' column if it exists
  ...
  await pool.query(`
    UPDATE attendance a SET
      ...
      ot_wage = CASE WHEN e.gender='female'
        THEN COALESCE((SELECT value::REAL FROM settings WHERE key='femaleOT'),50)
        ELSE COALESCE((SELECT value::REAL FROM settings WHERE key='maleOT'),100) END
  ...
  // Default settings
  const defs = { femaleDay: "350", femaleOT: "50", maleDay: "800", maleOT: "100" };
...
app.put("/api/settings", async (req, res) => {
  try {
    ...
    // If wage/OT settings changed — auto-recalculate ALL unpaid attendance records
    const wageKeys = ["femaleDay","femaleOT","maleDay","maleOT"];
    const changedWage = Object.keys(req.body).some(k => wageKeys.includes(k));
    if (changedWage) {
      // Get latest settings
      const settings = {};
      (await pool.query("SELECT key,value FROM settings")).rows.forEach(r => { settings[r.key] = Number(r.value); });

      // Update wages on all unpaid records (paid_amount = 0 only — never touch paid days)
      await pool.query(`
        UPDATE attendance SET
          daily_wage = CASE
            WHEN e.daily_wage > 0 THEN e.daily_wage
            WHEN e.gender='female' THEN $1
            ELSE $2
          END,
          ot_wage = CASE
            WHEN e.gender='female' THEN $3
            ELSE $4
          END
        FROM employees e
        WHERE attendance.emp_id = e.id
        AND attendance.paid_amount = 0
      `, [settings.femaleDay||350, settings.maleDay||800, settings.femaleOT||50, settings.maleOT||100]);

      // Recompute daily_salary for unpaid records
      await pool.query(`
        UPDATE attendance SET daily_salary =
          CASE
            WHEN status='present'  THEN daily_wage
    ...
    res.json({ ok: true, recalculated: changedWage });
  } catch(e) {
...
app.post("/api/attendance", async (req, res) => {
  try {
    ...
    if (existing && Number(existing.paid_amount) > 0 && Number(existing.paid_amount) >= Number(existing.daily_salary)) {
    ...
    const otWage = emp.gender === "female" ? (settings.femaleOT || 50) : (settings.maleOT || 100);
    const dailySalary = computeSalary(status, dailyWage, otWage);

    // UPSERT — paid_amount preserved on update
    await pool.query(`
...
app.post("/api/attendance/bulk", async (req, res) => {
  try {
    ...
    for (const eid of emp_ids) {
      ...
      if (existing && existing.paid_amount >= existing.daily_salary && existing.daily_salary > 0) continue;
      const dw = emp.daily_wage > 0 ? emp.daily_wage : (emp.gender === "female" ? (settings.femaleDay || 350) : (settings.maleDay || 800));
      const ow = emp.gender === "female" ? (settings.femaleOT || 50) : (settings.maleOT || 100);
...
app.post("/api/attendance/pay-full-range", async (req, res) => {
  try {
    ...
    if (total > 0) {
      // Include 'date' column for backward compat with old DB schema that has NOT NULL on date
      await pool.query(
        `INSERT INTO payments(id,emp_id,date,paid_date,amount,period_from,period_to,period_label,note,type)
         VALUES($1,$2,$3,$3,$4,$5,$6,$7,$8,'payment')
    ...
    console.error("pay-full-range error:", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.post("/api/attendance/pay-bulk", async (req, res) => {
  ...
  if (totalPaid > 0) {
    const paidDate2 = new Date().toISOString().slice(0, 10);
    await pool.query(
      `INSERT INTO payments(id,emp_id,date,paid_date,amount,period_from,period_to,period_label,note,type)
       VALUES($1,$2,$3,$3,$4,$5,$6,$7,$8,'payment')
       ON CONFLICT DO NOTHING`,
      [uid(), emp_id, paidDate2, totalPaid, from, to, period_label || `${from} \u2192 ${to}`, ""]
    );
  }
...
app.get("/api/salary/history", async (req, res) => {
  try {
    ...
    // Get all payments (or for one employee)
    const payQuery = emp_id
      ? "SELECT * FROM payments WHERE emp_id=$1 ORDER BY id DESC"
      : "SELECT * FROM payments ORDER BY id DESC";
    const payParams = emp_id ? [emp_id] : [];
    const { rows: allPayments } = await pool.query(payQuery, payParams);

    if (allPayments.length === 0) return res.json([]);

    // Get unique employee IDs from payments
    const empIds = [...new Set(allPayments.map(p => p.emp_id))];

    // Get employee info
    const empRows = (await pool.query(
      "SELECT * FROM employees WHERE id = ANY($1::text[])",
      [empIds]
    )).rows;

    const result = [];
    for (const emp of empRows) {
      const payments = allPayments.filter(p => p.emp_id === emp.id);
      if (payments.length === 0) continue;

      // Normalize each payment
      const normalized = payments.map(function(p) {
        // Resolve effective date — try paid_date first, then date column
        const effDate = (p.paid_date && p.paid_date !== '') ? p.paid_date
                      : (p.date && p.date !== '') ? p.date
                      : p.period_from || today;

        // Resolve label
        let lbl = (p.period_label && p.period_label !== '') ? p.period_label : '';
        if (!lbl && p.period_from && p.period_to) {
          lbl = p.period_from === p.period_to
            ? p.period_from
            : p.period_from + ' \u2192 ' + p.period_to;
        }
        if (!lbl && p.period) lbl = p.period;
        if (!lbl) lbl = effDate;
        ...
        return {
          ...
          period_from: p.period_from || effDate,
          period_to: p.period_to || effDate,
          period_label: lbl,
          note: p.note || '',
          type: p.type || 'payment'
        };
      ...
      const weekTotal  = normalized.filter(p => p.paid_date >= monday).reduce((s,p) => s+p.amount, 0);
      const monthTotal = normalized.filter(p => p.paid_date >= monthStart).reduce((s,p) => s+p.amount, 0);
      const yearTotal  = normalized.filter(p => p.paid_date >= yearStart).reduce((s,p) => s+p.amount, 0);
      const allTotal   = normalized.reduce((s,p) => s+p.amount, 0);
      ...
      result.push({
        ...
        phone: emp.phone || '',
        payments: normalized,
        weekTotal, monthTotal, yearTotal, allTotal
      });
    }
    ...
    res.status(500).json({ error: "History failed: " + e.message });
  }
});
...
// ══════════════════════════════════════════════════════════════════
// ── TRADE ENDPOINTS with year filter ─────────────────────────────

app.get("/api/purchases", async (req, res) => {
  const { year } = req.query;
  let sql = "SELECT p.*,v.name as variety_name,v.name_ta as variety_ta FROM purchases p LEFT JOIN varieties v ON p.variety_id=v.id";
  if (year) sql += ` WHERE p.date LIKE '${year}-%'`;
...
app.get("/api/sales", async (req, res) => {
  const { year } = req.query;
  let sql = "SELECT s.*,v.name_ta as variety_ta FROM sales s LEFT JOIN varieties v ON s.variety_id=v.id";
  if (year) sql += ` WHERE s.date LIKE '${year}-%'`;
...
app.get("/api/processing", async (req, res) => {
  const { year } = req.query;
  let sql = "SELECT p.*,v.name_ta as variety_ta FROM processing p LEFT JOIN varieties v ON p.variety_id=v.id";
  if (year) sql += ` WHERE p.date LIKE '${year}-%'`;
  sql += " ORDER BY p.date DESC";
  const { rows } = await pool.query(sql);
  res.json(rows);
...
app.get("/api/market-sales", async (req, res) => {
  const { year } = req.query;
  let sql = "SELECT m.*,v.name_ta as variety_ta FROM market_sales m LEFT JOIN varieties v ON m.variety_id=v.id";
  if (year) sql += ` WHERE m.date LIKE '${year}-%'`;
...
// ── DASHBOARD with year filter ─────────────────────────────────────
app.get("/api/dashboard", async (req, res) => {
  const { year } = req.query;
  const w = year ? ` WHERE date LIKE '${year}-%'` : "";
  const wp = year ? ` WHERE status='done' AND date LIKE '${year}-%'` : " WHERE status='done'";
  const wm = year ? ` WHERE date LIKE '${year}-%'` : "";
  const q = async (sql) => Number((await pool.query(sql)).rows[0].t);
  const tp=await q("SELECT COALESCE(SUM(final_weight),0) as t FROM purchases"+w);
  const ts=await q("SELECT COALESCE(SUM(total_weight),0) as t FROM sales"+w);
  const tpr=await q("SELECT COALESCE(SUM(raw_used),0) as t FROM processing"+wp);
  const tpq=await q("SELECT COALESCE(SUM(final_weight),0) as t FROM processing"+wp);
  const tms=await q("SELECT COALESCE(SUM(processed_qty),0) as t FROM market_sales"+wm);
  const trev=await q("SELECT COALESCE(SUM(revenue),0) as t FROM market_sales"+wm);
  const tpa=await q("SELECT COALESCE(SUM(total_amount),0) as t FROM purchases"+w);
  const tsa=await q("SELECT COALESCE(SUM(total_amount),0) as t FROM sales"+w);
...
// ── TRADE HISTORY — all years summary ─────────────────────────────
app.get("/api/trade/history", async (req, res) => {
  try {
    // Get all distinct years from all trade tables
    const yrs = await pool.query(`
      SELECT DISTINCT substr(date,1,4) as yr FROM (
        ...
        UNION SELECT date FROM market_sales
      ) t WHERE date IS NOT NULL AND date != '' ORDER BY yr DESC
    `);
    const years = yrs.rows.map(r => r.yr).filter(Boolean);

    const result = [];
    for (const year of years) {
      const w = `date LIKE '${year}-%'`;
      const q = async (sql) => Number((await pool.query(sql)).rows[0].t);

      const [purchases, sales, markets] = await Promise.all([
        pool.query(`SELECT p.*,v.name_ta as variety_ta FROM purchases p LEFT JOIN varieties v ON p.variety_id=v.id WHERE p.${w} ORDER BY p.date DESC`),
        pool.query(`SELECT s.*,v.name_ta as variety_ta FROM sales s LEFT JOIN varieties v ON s.variety_id=v.id WHERE s.${w} ORDER BY s.date DESC`),
        pool.query(`SELECT m.*,v.name_ta as variety_ta FROM market_sales m LEFT JOIN varieties v ON m.variety_id=v.id WHERE m.${w} ORDER BY m.date DESC`)
      ]);

      purchases.rows.forEach(r => { r.loads = JSON.parse(r.loads_json||"[]"); });
      sales.rows.forEach(r => { r.bags = JSON.parse(r.bags_json||"[]"); });

      const totalBought  = purchases.rows.reduce((s,r)=>s+Number(r.final_weight||0),0);
      const totalSeedSold= sales.rows.reduce((s,r)=>s+Number(r.total_weight||0),0);
      const totalRevenue = markets.rows.reduce((s,r)=>s+Number(r.revenue||0),0);
      const totalPurchaseAmt = purchases.rows.reduce((s,r)=>s+Number(r.total_amount||0),0);
      const totalSeedAmt     = sales.rows.reduce((s,r)=>s+Number(r.total_amount||0),0);
      ...
      result.push({
        ...
        summary: {
          ...
          remainingRaw: totalBought - totalSeedSold
        }
      });
    }
    res.json(result);
...
app.get("/api/reports/profit", async (req, res) => {
  ...
  const w = year ? ` WHERE date LIKE '${year}-%'` : "";
  const q = async (sql) => Number((await pool.query(sql)).rows[0].t);
  const tp=await q("SELECT COALESCE(SUM(total_amount),0) as t FROM purchases"+w);
  const ts=await q("SELECT COALESCE(SUM(total_amount),0) as t FROM sales"+w);
  const tpk=await q("SELECT COALESCE(SUM(final_weight),0) as t FROM purchases"+w);
  const tsk=await q("SELECT COALESCE(SUM(total_weight),0) as t FROM sales"+w);
  res.json({totalPurchase:tp,totalSale:ts,profit:ts-tp,stockKg:tpk-tsk});
});

app.get("/api/reports/stock", async (req, res) => {
  const { year } = req.query;
  const w = year ? ` AND date LIKE '${year}-%'` : "";
  const varieties=(await pool.query("SELECT * FROM varieties WHERE active=true")).rows;
  const result=[];
  for(const v of varieties){
    const bought=Number((await pool.query(`SELECT COALESCE(SUM(final_weight),0) as t FROM purchases WHERE variety_id=$1${w}`,[v.id])).rows[0].t);
    const sold=Number((await pool.query(`SELECT COALESCE(SUM(total_weight),0) as t FROM sales WHERE variety_id=$1${w}`,[v.id])).rows[0].t);
...
// Catch unhandled promise rejections — prevent Railway restart
