...
app.get("/api/salary/history", async (req, res) => {
  ...
  const result = [];
  for (const emp of emps) {
    // Robust query: handle both old (date) and new (paid_date) schemas
    const { rows: payments } = await pool.query(
      `SELECT *,
        COALESCE(NULLIF(paid_date,''), NULLIF(date,''), period_from, $2) as effective_date
       FROM payments
       WHERE emp_id=$1
       ORDER BY COALESCE(NULLIF(paid_date,''), NULLIF(date,''), period_from) DESC NULLS LAST`,
      [emp.id, today]
    );
    if (payments.length === 0) continue;

    // Normalize each payment for UI display
    const normalized = payments.map(function(p) {
      var effDate = p.paid_date || p.date || p.period_from || today;
      var lbl = p.period_label || "";
      if (!lbl && p.period_from && p.period_to && p.period_from !== p.period_to) {
        lbl = p.period_from + " \u2192 " + p.period_to;
      } else if (!lbl && p.period_from) {
        lbl = p.period_from;
      } else if (!lbl && p.period) {
        lbl = p.period; // old period column
      }
      return Object.assign({}, p, { paid_date: effDate, period_label: lbl });
    });

    const weekTotal  = normalized.filter(function(p){return (p.paid_date||"") >= monday;}).reduce(function(s,p){return s+Number(p.amount);}, 0);
    const monthTotal = normalized.filter(function(p){return (p.paid_date||"") >= monthStart;}).reduce(function(s,p){return s+Number(p.amount);}, 0);
    const yearTotal  = normalized.filter(function(p){return (p.paid_date||"") >= yearStart;}).reduce(function(s,p){return s+Number(p.amount);}, 0);
    const allTotal   = normalized.reduce(function(s,p){return s+Number(p.amount);}, 0);
