...
app.post("/api/attendance/pay-full-range", async (req, res) => {
  ...
  if (total > 0) {
    await pool.query(
      ...
      [uid(), emp_id, new Date().toISOString().slice(0, 10), total, from, to, period_label || `${from} முதல் ${to}`, ""]
    );
  }
...
app.get("/api/salary/history", async (req, res) => {
  ...
  for (const emp of emps) {
    const { rows: payments } = await pool.query(
      "SELECT * FROM payments WHERE emp_id=$1 ORDER BY paid_date DESC",
      [emp.id]
    );
    if (payments.length === 0) continue;

    const weekTotal  = payments.filter(p => p.paid_date >= monday).reduce((s, p) => s + Number(p.amount), 0);
    const monthTotal = payments.filter(p => p.paid_date >= monthStart).reduce((s, p) => s + Number(p.amount), 0);
    const yearTotal  = payments.filter(p => p.paid_date >= yearStart).reduce((s, p) => s + Number(p.amount), 0);
