...
app.get("/api/salary/history", async (req, res) => {
  ...
  const result = [];
  for (const emp of emps) {
    // Use COALESCE to handle old records with empty paid_date — fallback to 'date' column
    const { rows: payments } = await pool.query(
      `SELECT *,
        COALESCE(NULLIF(paid_date,''), NULLIF(date,''), period_from, today()) as display_date
       FROM payments
       WHERE emp_id=$1
    ...
    // Normalize each payment's date for filtering
    const normalized = payments.map(p => ({
      ...p,
      paid_date: p.paid_date || p.date || p.period_from || today,
      period_label: p.period_label || (p.period_from && p.period_to ? `${p.period_from} → ${p.period_to}` : (p.period || ''))
    }));
